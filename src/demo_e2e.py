...
def start_proc(module_name):
    # Starts a cli as a subprocess
    return subprocess.Popen([PY, "-m", module_name], stdin=subprocess.PIPE, text=True, bufsize=1)
...
def start_server():
    ...
    server = subprocess.Popen([PY, "-m", "src.server"])
